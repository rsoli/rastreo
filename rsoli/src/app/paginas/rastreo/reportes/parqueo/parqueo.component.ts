...
export class ParqueoComponent implements OnInit {
  ...
  EjecutarFiltros(){
    ...
    this.monitoreo_servicio.reporte_parqueos({id_vehiculos:this.vehiculo_seleccionado.id_vehiculo,fecha_inicio:f_ini,fecha_fin:f_fin}).subscribe(data=>{
      this.loading = false;

      this.closeLoading_alert();
      this.lista_recorrido=JSON.parse(JSON.stringify(data)).lista_parqueos_vehiculos;
